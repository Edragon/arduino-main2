...
class WAVHeader {
 ...
 protected:
  ...
  void writeRiffHeader(BaseBuffer<uint8_t> &buffer,
    ...
    LOGI("writeRiffHeader: file_size=%u riff_size=%u", (unsigned int)info.file_size, (unsigned int)chunk_size);
  ...
  void writeDataHeader(BaseBuffer<uint8_t> &buffer, const WAVAudioInfo &info) {
    ...
    LOGI("writeDataHeader: data_length=%u", (unsigned int)data_length);
