...
// TIMER_BASE_CLK may be missing in newer ESP32 cores. Use APB clock fallback when needed.
#if defined(TIMER_BASE_CLK)
  #define ESP32_ISR_SERVO_TIMER_BASE_CLK    TIMER_BASE_CLK
#elif defined(APB_CLK_FREQ)
  #define ESP32_ISR_SERVO_TIMER_BASE_CLK    APB_CLK_FREQ
#else
  #define ESP32_ISR_SERVO_TIMER_BASE_CLK    80000000UL
#endif

#define TIMER_SCALE               (ESP32_ISR_SERVO_TIMER_BASE_CLK / TIMER_DIVIDER)  // convert counter value to seconds
...
class ESP32TimerInterrupt
{
  ...
  public:
    ...
    ESP32TimerInterrupt(uint8_t timerNo)
      ...
      stdConfig.counter_en   = TIMER_START;          //starts counting counter once timer_init called
      stdConfig.intr_type    = TIMER_INTR_MAX;
      stdConfig.counter_dir  = TIMER_COUNT_UP;       //counts from 0 to counter value
      stdConfig.auto_reload  = TIMER_AUTORELOAD_EN;  //reloads counter automatically
    #if defined(SOC_TIMER_GROUP_SUPPORT_XTAL) && SOC_TIMER_GROUP_SUPPORT_XTAL
      stdConfig.clk_src      = TIMER_SRC_CLK_APB;    //Use APB as source clock
    #endif
      stdConfig.divider      = TIMER_DIVIDER;
    ...
    bool setFrequency(const float& frequency, timer_callback callback)
    {
      ...
      {      
        ...
        _frequency  = ESP32_ISR_SERVO_TIMER_BASE_CLK / TIMER_DIVIDER;   //1000000;
        _timerCount = (uint64_t) _frequency / frequency;
        // count up

#if USING_ESP32_S2_TIMERINTERRUPT
        ISR_SERVO_LOGERROR3(F("ESP32_S2_TimerInterrupt: _timerNo ="), _timerNo, F(", _fre ="), ESP32_ISR_SERVO_TIMER_BASE_CLK / TIMER_DIVIDER);
        ISR_SERVO_LOGERROR3(F("TIMER_BASE_CLK ="), ESP32_ISR_SERVO_TIMER_BASE_CLK, F(", TIMER_DIVIDER ="), TIMER_DIVIDER);
        ...
        ISR_SERVO_LOGERROR3(F("ESP32_S3_TimerInterrupt: _timerNo ="), _timerNo, F(", _fre ="), ESP32_ISR_SERVO_TIMER_BASE_CLK / TIMER_DIVIDER);
        ISR_SERVO_LOGERROR3(F("TIMER_BASE_CLK ="), ESP32_ISR_SERVO_TIMER_BASE_CLK, F(", TIMER_DIVIDER ="), TIMER_DIVIDER);
        ISR_SERVO_LOGERROR3(F("_timerIndex ="), _timerIndex, F(", _timerGroup ="), _timerGroup);
        ISR_SERVO_LOGERROR3(F("_count ="), (uint32_t) (_timerCount >> 32) , F("-"), (uint32_t) (_timerCount));
        ISR_SERVO_LOGERROR1(F("timer_set_alarm_value ="), TIMER_SCALE / frequency);        
#else
        ISR_SERVO_LOGERROR3(F("ESP32_TimerInterrupt: _timerNo ="), _timerNo, F(", _fre ="), ESP32_ISR_SERVO_TIMER_BASE_CLK / TIMER_DIVIDER);
        ISR_SERVO_LOGERROR3(F("TIMER_BASE_CLK ="), ESP32_ISR_SERVO_TIMER_BASE_CLK, F(", TIMER_DIVIDER ="), TIMER_DIVIDER);
        ISR_SERVO_LOGERROR3(F("_timerIndex ="), _timerIndex, F(", _timerGroup ="), _timerGroup);
        ISR_SERVO_LOGERROR3(F("_count ="), (uint32_t) (_timerCount >> 32) , F("-"), (uint32_t) (_timerCount));
        ISR_SERVO_LOGERROR1(F("timer_set_alarm_value ="), TIMER_SCALE / frequency);
